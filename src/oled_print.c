...
void OLED_PRT_putChr(char data, uint8_t x, uint8_t y, uint8_t size )
{
   ...
   if( x > (OLED_WIDTH - size * 5) ) return;

   for(uint8_t i = 0; i < 5; i++)
   {

      byte_from_flash = OLED_Font5x7[tab_index+i];

      for(uint8_t x = 0; x < 8; x++)
      {
          if( byte_from_flash & (1<<x) ) OLED_GFX_drawFilledSquare(tmp_x, tmp_y, size, OLED_COLOR_WHITE);
          else OLED_GFX_drawFilledSquare(tmp_x, tmp_y, size, OLED_COLOR_BLACK);
          tmp_y += size;
      }
      tmp_y = y;
      tmp_x += size;
...
void OLED_PRT_putStr(char * data, uint8_t x, uint8_t y, uint8_t size )
{
    ...
    while( (ascii = *data++) )
    {
        OLED_PRT_putChr(ascii,x,y,size);
        x += shift_x;
    }
}
...
void OLED_PRT_putInt(int32_t num , uint8_t x, uint8_t y, uint8_t size )
{
    ...
    OLED_PRT_putStr(temp_buffer,x,y,size);
...
void OLED_PRT_putFloat( float num , uint8_t precision, uint8_t x, uint8_t y, uint8_t size )
{
    ...
    *((uint32_t *)(&num)) &= ~(1<<31);
    ...
    OLED_PRT_putStr(f.wsk,x,y,size);
